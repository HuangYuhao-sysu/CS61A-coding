...
class Name(Expr):
    ...
    def eval(self, env):
        ...
        if self.string in env:
            ...
            raise NameError('Can\'t find the name!')
...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        """
        "*** YOUR CODE HERE ***"
        eval_opertr = self.operator.eval(env)
        eval_operds = [operd.eval(env) for operd in self.operands]
        return eval_opertr.apply(eval_operds)
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        for i in range(len(self.parameters)):
            local[self.parameters[i]]=arguments[i]
