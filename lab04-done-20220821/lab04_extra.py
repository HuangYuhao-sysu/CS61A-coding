...
def merge(lst1, lst2):
    ...
    for i in range(lgth-1):
        for j in range(lgth-1-i):
            if orgn[j] > orgn[j+1]:
                orgn[j], orgn[j+1] = orgn[j+1], orgn[j]
    return orgn
...
def print_board(board, max_rows, max_cols):
    ...
    for curr_row in range(max_rows):
        total_row = ''
        for curr_col in range(max_cols):
            total_row = total_row + get_piece(board, curr_row, curr_col) + ' '
        print(total_row.strip())
...
def check_win_row(board, max_rows, max_cols, num_connect, row, player):
    ...
    cnt = 0
    for curr_col in range(max_cols):
        curr_ele = get_piece(board, row, curr_col)
        if curr_ele == player:
            cnt += 1
        else:
            if cnt >= num_connect:
...
def check_win_column(board, max_rows, max_cols, num_connect, col, player):
    ...
    cnt = 0
    for curr_row in range(max_rows):
        curr_ele = get_piece(board, curr_row, col)
        if curr_ele == player:
            cnt += 1
        else:
            if cnt >= num_connect:
