...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        range = 0
        while find_place.entrance != None:
            if find_place.bees:
                if self.min_range <= range <= self.max_range:
                    return random_or_none(find_place.bees)
                else:
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        if self.ant and not self.ant.watersafe:
            self.ant.reduce_armor(self.ant.armor)
        org_bees = self.bees[:]
        for bee in org_bees:
            if not bee.watersafe:
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt do nothing but have a high armor."""
...
class HungryAnt(Ant):
    ...
    def __init__(self):
        # BEGIN Problem 6
        "*** YOUR CODE HERE ***"
        self.digesting = 0 #left turns to digest
        Ant.__init__(self, 1)
        # END Problem 6
...
# END Problem 13
    ...
    def __init__(self):
        ...
        "*** YOUR CODE HERE ***"
        Ant.__init__(self, 1)
        if QueenAnt.has_true:
            self.is_true = False
        else:
            self.is_true = True
            QueenAnt.has_true = True
        self.ants_have_doubled = []
        # END Problem 13
    ...
    def action(self, colony):
        ...
        if self.is_true:
            ...
            find_place = self.place.exit
            while find_place != None:
                if find_place.ant and not isinstance(find_place.ant, BodyguardAnt) and find_place.ant not in self.ants_have_doubled:
                    find_place.ant.damage *= 2
                    self.ants_have_doubled.append(find_place.ant)
                elif find_place.ant and isinstance(find_place.ant, BodyguardAnt):
                    if find_place.ant not in self.ants_have_doubled:
                        find_place.ant.damage *= 2
                        self.ants_have_doubled.append(find_place.ant)
                    if find_place.ant.ant and find_place.ant.ant not in self.ants_have_doubled:
                        find_place.ant.ant.damage *= 2
                        self.ants_have_doubled.append(find_place.ant.ant)
                find_place = find_place.exit
        else:
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    "*** YOUR CODE HERE ***"
    org_action = bee.action
    new_action = effect(bee.action)

    def action(colony):
        nonlocal duration
        if duration == 0:
            return org_action(colony)
        else:
            duration -= 1
